...
<main class="form-signin w-100 m-auto">
  <form method="post" action="">
    ...
    <div class="form-floating">
      <input type="taxt" class="form-control" name="usr" placeholder="Username" required>
...
if(isset($_POST['Submit'])) {
    ...
    $sql = "SELECT * FROM admin WHERE a_user = ? LIMIT 1";
    $stmt = mysqli_prepare($conn, $sql);

    if (!$stmt) {
        die("ข้อผิดพลาดของ SQL: " . mysqli_error($conn)); // ตรวจสอบข้อผิดพลาด
    }
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        if (password_verify($pwd, $row['a_pwd'])) { // ตรวจสอบรหัสผ่าน
